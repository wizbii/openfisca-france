...
def split(scenario):
    ...
    year = scenario.year
    # Contient en réalité les détachements possibles puisqu'au départ tous les membres sous rattachés au même foyer
    ...
    for pac_index, pac_id in enumerate(foyer_fiscal.pop('personnes_a_charge')):
        ...
        # Exprime la condition de rattachement au foyer pour les majeurs
    ...
    for i in range(0, n):
        ...
        scenarios[i].test_case['foyers_fiscaux'][0]['personnes_a_charge'] = foyers_possibles[i] + \
            detachements_impossibles
    ...
    print "Le plus avantageux pour votre famille est que les jeunes rattachés à votre foyer fiscal soient : {}. " \
        "Vous paierez alors {}€ d'impôts. (Seuls les jeunes éligibles au rattachement sont indiqués " \
        "(18 <= age < 21 si pas étudiant / 25 sinon. Le calculateur a émis l'hypothèse qu'il n'y avait qu'un seul " \
        "foyer fiscal au départ, auquel tous les jeunes éligibles étaient rattachés.)".format(
            foyers_possibles[min_[0]], min_[1])
    return impots


# Calcule l'ensemble des parties des éléments d'un array, sous forme d'un array d'arrays
def partiesDe(tab):
    n = len(tab)
    if n == 0:
        return [[]]
...
# Concatène un array d'arrays (tab) avec le même array où un élément (a) aura été rajouté à chaque sous-array
def add(a, tab):
    n = len(tab)
    for i in range(0, n):
        b = list(tab[i])
