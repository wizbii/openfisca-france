...
class cotsyn(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        '''
        f7ac_i = foyer_fiscal.members('f7ac', period)
        sal_i = foyer_fiscal.members('salaire_imposable', period, options = [ADD])
        cho_i = foyer_fiscal.members('chomage_imposable', period, options = [ADD])
        rst_i = foyer_fiscal.members('retraite_imposable', period, options = [ADD])

        P = parameters(start).impot_revenu.reductions_impots.cotsyn
        tx = P.seuil

        max_i = (sal_i + cho_i + rst_i) * tx
